...
class RecurrenceDetector:
    ...
    def normalize_title(self, title: str) -> str:
        ...
        normalized = self.YEAR_PATTERN.sub("", normalized)
        normalized = re.sub(r"[^\w\s]", " ", normalized, flags=re.UNICODE)
        normalized = re.sub(r"\s+", " ", normalized).strip()
        return normalized

    def build_parent_event_key(self, title: str, source_domain: str) -> str:
        return f"{source_domain}:{self.normalize_title(title)}"
